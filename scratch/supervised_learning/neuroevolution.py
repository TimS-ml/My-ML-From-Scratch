...
class Neuroevolution():
    ...
    def _build_model(self, id):
        ...
        model = self.model_builder(n_inputs=self.X.shape[1],
    ...
    def _mutate(self, individual, var=1):
        ...
        for layer in individual.layers:
            if hasattr(layer, 'W'):
                ...
                mutation_mask = np.random.binomial(1,
                                                   p=self.mutation_rate,
                                                   size=layer.W.shape)
                layer.W += np.random.normal(
                    loc=0, scale=var, size=layer.W.shape) * mutation_mask
                mutation_mask = np.random.binomial(1,
                                                   p=self.mutation_rate,
                                                   size=layer.w0.shape)
                layer.w0 += np.random.normal(
                    loc=0, scale=var, size=layer.w0.shape) * mutation_mask
    ...
    def _crossover(self, parent1, parent2):
        ...
        for i in range(len(child1.layers)):
            if hasattr(child1.layers[i], 'W'):
                ...
                child1.layers[i].W[:, cutoff:] = parent2.layers[
                    i].W[:, cutoff:].copy()
                child1.layers[i].w0[:, cutoff:] = parent2.layers[
                    i].w0[:, cutoff:].copy()
                child2.layers[i].W[:, cutoff:] = parent1.layers[
                    i].W[:, cutoff:].copy()
                child2.layers[i].w0[:, cutoff:] = parent1.layers[
                    i].w0[:, cutoff:].copy()
    ...
    def evolve(self, X, y, n_generations):
        ...
        for epoch in range(n_generations):
            ...
            sorted_i = np.argsort([model.fitness
            ...
            print("[%d Best Individual - Fitness: %.5f, Accuracy: %.1f%%]" %
                  (epoch, fittest_individual.fitness,
                   float(100 * fittest_individual.accuracy)))
            # The 'winners' are selected for the next generation
            next_population = [self.population[i] for i in range(n_winners)]

            total_fitness = np.sum(
                [model.fitness for model in self.population])
