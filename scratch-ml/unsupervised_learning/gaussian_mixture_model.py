...
from scratch.utils import normalize, euclidean_distance, calculate_covariance_matrix
from scratch.utils import Plot
...
class GaussianMixtureModel():
    ...
    def multivariate_gaussian(self, X, params):
        ...
        for i, sample in enumerate(X):
            ...
            coeff = (1.0 / (math.pow(
                (2.0 * math.pi), d / 2) * math.sqrt(determinant)))
            exponent = math.exp(
                -0.5 * (sample - mean).T.dot(np.linalg.pinv(covar)).dot(
                    (sample - mean)))
    ...
    def _get_likelihoods(self, X):
        ...
        for i in range(self.k):
            likelihoods[:,
                        i] = self.multivariate_gaussian(X, self.parameters[i])
        return likelihoods
    ...
    def _expectation(self, X):
        ...
        sum_likelihoods = np.expand_dims(np.sum(weighted_likelihoods, axis=1),
                                         axis=1)
    ...
    def _maximization(self, X):
        ...
        for i in range(self.k):
            ...
            self.parameters[i]["mean"], self.parameters[i][
                "cov"] = mean, covariance
    ...
    def _converged(self, X):
        ...
        if len(self.responsibilities) < 2:
            return False
        diff = np.linalg.norm(self.responsibilities[-1] -
